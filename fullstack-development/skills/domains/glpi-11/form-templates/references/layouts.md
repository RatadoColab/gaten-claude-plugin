# GLPI 11 Form Templates — Layouts
...
<form method="post" action="{{ path('/plugins/nomedoplugin/MeuItem') }}">
    ...
    <div class="card-footer d-flex justify-content-end gap-2 mt-3">
        ...
        <a href="{{ path('/plugins/nomedoplugin/MeuItem') }}" class="btn btn-outline-secondary">
...
Se o formulário ainda submete para um `front/item.form.php` legado (não migrado para Controller), usar `path('/plugins/nomedoplugin/front/item.form.php')` no lugar da rota — a função `get_plugin_web_dir()` está depreciada em ambos os casos.

---

## Catálogo de Macros de Campo

**Idêntico ao GLPI 10** — nenhuma macro de campo mudou de assinatura.
...
> **Atenção:** no GLPI 11, sem auto-sanitização, o que está salvo em `item.fields['content']` é exatamente o que o usuário enviou no `save()` — vir do banco **não** torna o conteúdo seguro. `item.fields['content']|raw` só é aceitável aqui se o valor já foi sanitizado/escapado **no momento de salvar** (ex.: `symfony/html-sanitizer` para campos rich text, ou `htmlescape()` para texto simples reexibido como HTML). Nunca aplicar `|raw` a um campo que armazena input de usuário sem essa sanitização na gravação — a auto-sanitização do GLPI 10 que tornava isso seguro por padrão não existe mais.

---

## Layout 1: Formulário de Configuração com Múltiplos Cards

Padrão para formulários de configuração de plugin, agora roteado por Controller.
...
<form method="post" action="{{ path('/plugins/nomedoplugin/Config') }}">
    <input type="hidden" name="_glpi_csrf_token" value="{{ csrf_token() }}" />
...
**Inalterado em relação ao GLPI 10** — padrão para `showForm()` em `src/MinhaClasse.php`.
...
Estrutura Bootstrap 5 idêntica ao GLPI 10; a chamada AJAX aponta para a rota do Controller (recomendado) ou para o handler legado em `ajax/`, que continua acessível pela mesma URL.
...
$('#btn-clone-submit-{{ rand }}').on('click', function() {
    ...
    $.ajax({
        url:      '{{ path("/plugins/nomedoplugin/MeuItem/Clone") }}',
        type:     'POST',
...
Estrutura idêntica ao GLPI 10; só a `action`/URL do submit muda para a rota do Controller quando migrado.
...
<div id="wizard-{{ rand }}" class="card-group">
    <div class="wizard-container">
        <form method="post" id="wizard-form-{{ rand }}"
              action="{{ path('/plugins/nomedoplugin/Wizard') }}">
            <input type="hidden" name="_glpi_csrf_token" value="{{ csrf_token() }}" />
