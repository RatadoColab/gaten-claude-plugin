---
name: glpi-10-plugin-creation
description: This skill should be loaded when the user asks to "create a GLPI plugin", "start a new plugin", "scaffold a GLPI module", "generate the initial plugin structure", "create setup.php", "create hook.php", "create install.php", or "create the plugin main class" — and the target is GLPI 10.0.x (explicit "GLPI 10" mention, or no version stated and the user confirmed 10.x when asked). For GLPI 11 targets, use `domains/glpi-11/plugin-creation/SKILL.md` instead. If the GLPI version cannot be determined from the project or the user's message, ask which version before generating code — do not assume a default. Provides a complete step-by-step guide for creating a valid GLPI 10.0.x plugin from scratch, including required files, hook registration, initial CommonDBTM class, and install script.
---

# Criação de Plugin GLPI 10.0.x

> **Versão-alvo:** GLPI 10.0.x. Para GLPI 11, usar `domains/glpi-11/plugin-creation/SKILL.md`.
...
Seguir a tabela **Nomenclatura** da skill pai (`domains/glpi-10/SKILL.md`): diretório, tabelas, classes, direitos e funções de hooks/setup.
...
- **`../references/architecture.md`** — CommonDBTM, `$DB`, `Session`, hooks disponíveis, segurança, licença
